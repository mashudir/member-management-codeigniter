<script>
	...
	function submit(x){
      if (x=='add') {
        $('#btn-save').show();
        $('#btn-update').hide();
        $('#modal-title').html('<b>Add category</b>');
        ...
        $('#modal-title').html('<b>Update category</b>');

        $.ajax({
          type:"GET",
          data:'id='+x,
          url:'<?php echo base_url('master/get_kategory') ?>',
          dataType:'JSON',
    ...
    function add(){
      if ($("[name='name']").val()=='') {
        ...
        $.ajax({
          ...
          error : function(err) {
            var html = "<div class='alert alert-danger'>Failed to add category!</div>";
    ...
    function update(){
      if ($("[name='name']").val()=='') {
        ...
        $.ajax({
          ...
          contentType: false,
          processData: false,
          url:'<?php echo base_url('master/update_category') ?>',
          dataType : 'JSON',
          success : function(data){
              $("#form-add").modal('hide');
          ...
          error : function(err) {
            var html = "<div class='alert alert-danger'>Failed to update category!</div>";
              $('#alert-modal').html(html).fadeIn().delay(3000).fadeOut();
              $('#btn-update').text("Update");
              $('#btn-update').attr('disabled',false);
          }
