									...
									foreach ($this->data['users'] as $k => $v): ?>
										<tr>
											<textarea style="display: none;" id="<?php echo $v->id ?>">
												<?php 
												echo $v->id."|".$v->name."|".$v->username."|".
												$v->is_active."|".$v->role."|".$v->group_name."|".
												$v->group_id ?>
											</textarea>
											<td><?php echo $i ?></td>
											<td>
											...
											<td>
												<?php if ($v->role == 'kmm_klp') : ?>
													<div class="badge badge-warning">KMM Kelompok</div>
												<?php elseif ($v->role == 'kmm_desa'): ?>
													<div class="badge badge-success">KMM Desa</div>
												<?php else: ?>
													<div class="badge badge-info">PPD</div>
												<?php endif; ?>
											</td>
											<td class="text-center">
												<a href="#modal-add" data-toggle="modal" title="Update user info" onclick="submit(<?php echo $v->id; ?>)">
													<span class="text-warning"><i class="fa fa-pen"></i>&nbsp;&nbsp;&nbsp;&nbsp;</span>
												</a>
												<a href="#!" title="Delete user" onclick="deleteConfirm('<?php echo base_url('users/delete/'.$v->id) ?>')">
